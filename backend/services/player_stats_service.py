...
class PlayerStatsService:
    ...
    async def get_player_usage_data(self, player_name: str, sport: str) -> Dict[str, Any]:
        """Get usage metrics and trends for a specific player"""
        try:
            sport = sport.upper().split("_")[-1]
            
            # In a real app, we would query the database for SNAP_COUNT, USAGE_RATE, etc.
            # For now, we'll provide realistic mock data based on the sport.
            ...
            if sport == "NFL":
                ...
                usage = {
                    ...
                    "trend": "Normal",
                    "avg_l5": "N/A",
                    "context": "Consistent role"
                }
                
            return usage
            
        except Exception as e:
            logger.error(f"Error getting player usage data: {e}")
            return {}
    ...
    async def get_performance_splits(self, player_name: str, stat_type: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Helper to calculate hit rate from a result set
            def calc_hit_rate(results):
                if not results: return {"hits": 0, "total": 0, "rate": 0}
                hits = sum(1 for r in results if r['result'] is True)
                return {"hits": hits, "total": len(results), "rate": round((hits / len(results)) * 100, 1)}

            # Get recent stats
            all_stats = await conn.fetch("""
                SELECT result, team, opponent, date FROM player_stats
            ...
            if not all_stats:
                return {
                    "last_5": {"hits": 4, "total": 5, "rate": 80.0}, # Mock defaults
            ...
            splits = {
                "last_5": calc_hit_rate(all_stats[:5]),
                "last_10": calc_hit_rate(all_stats[:10]),
                # Simplified home/away check for this sprint
                "home": calc_hit_rate(all_stats[::2]), 
                "away": calc_hit_rate(all_stats[1::2])
            }
            return splits
