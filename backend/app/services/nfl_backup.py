"""
NFL odds backup utility for JSON file storage.

Saves daily backups of NFL odds data for fallback when APIs fail.
"""
...
def _get_backup_dir() -> Path:
    ...
    backup_dir = getattr(settings, 'nfl_backup_dir', None)
    if backup_dir:
...
def get_backup_filename(backup_date: date) -> str:
    ...
    Format: YYYY-MM-DD-NFL-odds.json
    """
    return f"{backup_date.isoformat()}-NFL-odds.json"
...
def save_backup(odds_data: list[dict[str, Any]], backup_date: Optional[date] = None) -> str:
    """
    Save NFL odds data to JSON backup file.
    ...
    try:
        ...
        logger.info(f"Saved NFL odds backup: {backup_path} ({len(odds_data)} records)")
...
def load_backup(backup_date: date) -> list[dict[str, Any]]:
    """
    Load NFL odds data from backup file for a specific date.
    ...
    try:
        ...
        logger.info(f"Loaded NFL odds backup: {backup_path} ({len(odds_data)} records)")
        return odds_data
...
def list_backups(limit: int = 30) -> list[dict[str, Any]]:
    ...
    backups = []
    for backup_file in sorted(backup_dir.glob("*-NFL-odds.json"), reverse=True)[:limit]:
        try:
            stat = backup_file.stat()
            # Extract date from filename
            date_str = backup_file.stem.replace("-NFL-odds", "")
            backups.append({
...
def delete_old_backups(days_to_keep: int = 90) -> int:
    ...
    for backup_file in backup_dir.glob("*-NFL-odds.json"):
        try:
            # Extract date from filename
            date_str = backup_file.stem.replace("-NFL-odds", "")
