...
async def sync_odds_for_sport(sport: str,
...
@router.post(" / odds")
async def sync_all_odds(db: AsyncSession = Depends(get_db),):
...
@router.post(" / rosters")
async def sync_rosters(sport: str = "NBA",
...
async def sync_single_player_roster(player_name: str,
 sport: str = "NBA",
 use_stubs: bool = False,
 db: AsyncSession = Depends(get_db),):
 """
 ...
 try:
 team_name = await sync_player_team_from_roster_api(db, player_name, sport_key, use_stubs = use_stubs)
...
async def sync_player_teams(sport: str = "basketball_nba",
 db: AsyncSession = Depends(get_db),):
 """
...
async def clear_games(sport: str = "basketball_nba",
 db: AsyncSession = Depends(get_db),):
 """
...
async def get_sync_status(db: AsyncSession = Depends(get_db),):
 ...
 current_lines = await db.scalar(select(func.count()).select_from(Line).where(Line.is_current =  = True))
 ...
 active_picks = await db.scalar(select(func.count()).select_from(ModelPick).where(ModelPick.is_active =  = True))
 players_count = await db.scalar(select(func.count()).select_from(Player))
 players_with_team = await db.scalar(select(func.count()).select_from(Player).where(Player.team_id.isnot(None)))
