...
async def count_matching_picks(db: AsyncSession,
 filters: dict,) - > int:
...
async def create_watchlist(data: WatchlistCreate,
 ...
 return WatchlistResponse(id = watchlist.id,
 name = watchlist.name,
...
async def list_watchlists(sport_id: Optional[int] = Query(None, description = "Filter by sport"),
 db: AsyncSession = Depends(get_db),):
 """
 ...
 items.append(WatchlistResponse(id = w.id,
 name = w.name,
 filters = w.filters,
...
async def mark_watchlist_checked(watchlist_id: int,
 db: AsyncSession = Depends(get_db),):
 """
