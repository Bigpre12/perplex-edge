...
async def get_market_analysis(sport_id: int = Query(30, description = "Sport ID(30 = NBA, 32 = NCAA, 53 = NHL)"),
 ...
 selectinload(ModelPick.market)).where(ModelPick.sport_id =  = sport_id).order_by(desc(ModelPick.expected_value)).limit(20)
...
async def get_trading_signals(min_confidence: float = Query(0.7, description = "Minimum confidence level"),
 db: AsyncSession = Depends(get_db)):
 """Get current trading signals."""
 try:
 # Get high confidence picks
 query = select(ModelPick).options(selectinload(ModelPick.player).selectinload(Player.team),
 selectinload(ModelPick.market)).where(ModelPick.confidence_score > = min_confidence).order_by(desc(ModelPick.confidence_score)).limit(10)
...
async def get_sportsbook_intelligence(sport_id: int = Query(None, description = "Sport ID filter"),
 db: AsyncSession = Depends(get_db)):
...
async def get_market_summary(db: AsyncSession = Depends(get_db)):
 ...
 query = select(ModelPick).options(selectinload(ModelPick.player).selectinload(Player.team),
 selectinload(ModelPick.market)).limit(100)
...
async def trigger_market_analysis(sport_id: int = Query(30, description = "Sport ID to analyze"),
 db: AsyncSession = Depends(get_db)):
