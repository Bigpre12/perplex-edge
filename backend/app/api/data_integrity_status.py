...
async def get_data_integrity_status(db: AsyncSession = Depends(get_db)):
 ...
 valid_picks_query = select(func.count(ModelPick.id)).where(ModelPick.expected_value.isnot(None),
 ...
 high_quality_picks_query = select(func.count(ModelPick.id)).where(ModelPick.expected_value > 0.05,
...
async def get_quality_report(db: AsyncSession = Depends(get_db)):
 ...
 sport_quality_query = select(func.count(ModelPick.id)).where(ModelPick.sport_id =  = sport_id,
