...
async def search_props(# Text filters
 ...
 # Build base query
 query = (select(Line, Player, PlayerTeam, Game, HomeTeam, AwayTeam, Market, ModelPick)
 ...
 .outerjoin(ModelPick,
 and_(ModelPick.game_id =  = Line.game_id,
 ...
 if team:
 query = query.where(or_(PlayerTeam.name.ilike(f"%{team}%"),
 ...
 if sport:
 sport_result = await db.execute(select(Sport).where(Sport.key =  = sport))
 ...
 injury_result = await db.execute(select(Injury)
 ...
 items.append(PropSearchResult(line_id = line.id,
 ...
 return PropSearchResponse(items = items,
 total = total,
 page = page,
 per_page = per_page,
 filters_applied = {
...
async def list_props(game_id: Optional[int] = Query(None, description = "Filter by game"),
 ...
 return LineList(items = [LineRead.model_validate(line) for line in lines],
 total = total or 0,)

@router.get(" / compare / {game_id} / {player_id}", response_model = list[LineComparison])
async def compare_player_props(game_id: int,
 player_id: int,
 ...
 bookmaker_lines = [
 BookmakerLine(sportsbook = line.sportsbook,
 odds = line.odds,
 ...
 comparisons.append(LineComparison(game_id = game_id,
...
async def list_players_with_props(game_id: int,
 db: AsyncSession = Depends(get_db),):
...
async def list_stat_types(sport: Optional[str] = Query(None, description = "Filter by sport"),
 db: AsyncSession = Depends(get_db),):
 """List available stat types for props."""
 query = (select(Market.stat_type)
 .where(Market.market_type =  = "player_prop",
