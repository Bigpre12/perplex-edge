...
from app.schemas.public import(PublicSport,
 PublicSportList,
 PublicGame,
 PublicGameList,
 PublicMarket,
...
async def get_book_lines_for_pick(db: AsyncSession,
 game_id: int,
 player_id: int,
 market_id: int,
 side: str,
 model_prob: float,) - > tuple[list[BookLine], str | None, float | None]:
 """
 ...
 result = await db.execute(select(Line).where(and_(Line.game_id =  = game_id,
 ...
 book_lines.append(BookLine(sportsbook = line.sportsbook,
...
async def list_sports(db: AsyncSession = Depends(get_db),):
 ...
 result = await db.execute(select(Sport).where(Sport.id.in_(valid_sport_ids)).order_by(Sport.name))
 sports = result.scalars().all()

 response = PublicSportList(items = [PublicSport.model_validate(s) for s in sports],
 total = len(sports),)
...
async def get_sport_availability(sport_id: int,
 ...
 picks_count = await db.scalar(select(func.count(ModelPick.id))
...
async def get_tonight_summary(db: AsyncSession = Depends(get_db),):
 """
 ...
 .where(and_(Game.sport_id =  = sport.id,
 Game.start_time > = today_utc,
 Game.start_time < tomorrow_utc,)))
 games_count = games_count_result.scalar() or 0

 # Count player props for this sport today
 props_result = await db.execute(select(func.count(ModelPick.id),
 ...
 if games_count > 0:
 sport_summaries.append(SportTonightSummary(sport_id = sport.id,
 ...
 return TonightSummaryResponse(date = now_et.strftime("%Y - %m - %d"),
 timezone = "US / Eastern",
...
async def get_hot_cold_players_endpoint(sport_id: Optional[int] = Query(None, description = "Filter by sport ID"),
 ...
 result = await get_hot_cold_players(db,
 sport_id = sport_id,
 min_picks = min_picks,
 limit = limit,)
...
async def get_hot_cold_players_by_market_endpoint(sport_id: Optional[int] = Query(None, description = "Filter by sport ID"),
 ...
 result = await get_hot_cold_players_by_market(db,
 sport_id = sport_id,
 market = market,
 side = side,
 min_picks = min_picks,
...
async def list_games_today(sport_id: int,
 ...
 result = await db.execute(select(Game, HomeTeam, AwayTeam)
 ...
 games.append(PublicGame(id = game.id,
...
async def list_markets(sport_id: int,
 ...
 return PublicMarketList(items = [PublicMarket.model_validate(m) for m in markets],
 total = len(markets),)
...
async def list_player_prop_picks(sport_id: int,
 ...
 injured_subquery = (select(Injury.player_id)
 ...
 query = (select(ModelPick, Player, PlayerTeam, Game, HomeTeam, AwayTeam, Market)
 ...
 lines_result = await db.execute(select(Line).where(or_( * line_conditions)))
 ...
 earliest_subq = (select(Line.game_id,
 Line.player_id,
 Line.market_id,
 Line.side,
 ...
 opening_lines_result = await db.execute(select(Line)
 .join(earliest_subq,
 and_(Line.game_id =  = earliest_subq.c.game_id,
 ...
 kelly = calculate_kelly_fraction(win_prob = pick.model_probability,
 ...
 picks.append(PlayerPropPick(pick_id = pick.id,
 ...
 logger.info(f"[player - props] sport_id = {sport_id} ({sport_key_debug}), fresh_only = {fresh_only}: "
 ...
 if mismatched:
 logger.warning(f"[SPORT_ID_MISMATCH] sport_id = {sport_id} request returned {len(mismatched)} picks "
 f"with different sport_ids: {set(getattr(p, 'sport_id', None) for p in mismatched)}")

 return PlayerPropPickList(items = picks,
...
async def list_game_line_picks(sport_id: int,
 ...
 query = (select(ModelPick, Game, HomeTeam, AwayTeam, Market)
 .join(Game, ModelPick.game_id =  = Game.id)
 .join(HomeTeam, Game.home_team_id =  = HomeTeam.id)
 .join(AwayTeam, Game.away_team_id =  = AwayTeam.id)
 .join(Market, ModelPick.market_id =  = Market.id)
 ...
 picks.append(GameLinePick(pick_id = pick.id,
 game_id = game.id,
 home_team = home_team.name,
 home_team_abbr = home_team.abbreviation,
 away_team = away_team.name,
 ...
 # Debug logging to track filtered / skipped picks
 logger.info(f"[game - lines] sport_id = {sport_id}, fresh_only = {fresh_only}: "
 f"returning {len(picks)} picks, total = {total}")

 return GameLinePickList(items = picks,
 total = total,
 filters = {
 "sport_id": sport_id,
...
async def debug_picks(sport_id: int,
 limit: int = Query(10, ge = 1, le = 100),
 db: AsyncSession = Depends(get_db),):
 """Enhanced debug endpoint to diagnose pick visibility issues."""
 # Get picks
 result = await db.execute(select(ModelPick)
 .where(ModelPick.sport_id =  = sport_id)
 .limit(limit))
 ...
 player_name = None
 if p.player_id:
 player_result = await db.execute(select(Player.id, Player.name).where(Player.id =  = p.player_id))
 player = player_result.first()
 player_exists = player is not None
 player_name = player[1] if player else None

 # Check game start_time
 game_result = await db.execute(select(Game.start_time, Game.status).where(Game.id =  = p.game_id))
 game = game_result.first()

 # Check if player is injured
 injury_status = None
 if p.player_id:
 injury_result = await db.execute(select(Injury.status).where(Injury.player_id =  = p.player_id))
...
async def list_100_percent_props(sport_id: int,
 ...
 response = HundredPercentPropList(items = props if props else [],
 ...
 return HundredPercentPropList(items = [],
 total = 0,
 window = window,)

# Convenience endpoint alias
@router.get(" / hitrate / 100", tags = ["public"])
async def hitrate_100_alias(sport: str = Query(..., description = "Sport key(nba, ncaab, nfl) or sport_id"),
 window: str = Query("last_5", description = "Time window: season, last_10, last_5"),
 limit: int = Query(50, ge = 1, le = 100, description = "Maximum results"),
 min_hit_rate: float = Query(0.70, ge = 0.0, le = 1.0, description = "Minimum hit rate fallback(0 - 1, default 0.70 = 70%)"),
 db: AsyncSession = Depends(get_db),):
 ...
 return HundredPercentPropList(items = props,
 total = len(props),
 window = window,)
 except Exception as e:
...
async def build_parlay(sport_id: int,
 ...
 return ParlayBuilderResponse(parlays = [],
 total_candidates = 0,
 leg_count = leg_count,
 filters_applied = {
...
async def auto_generate_slips(sport_id: int,
 ...
 try:
 result = await generate_slips(db,
 sport_id = sport_id,
 ...
 return AutoGenerateSlipsResponse(slips = result["slips"],
 ...
 from app.schemas.public import AutoGenerateSlipsResponse
 return AutoGenerateSlipsResponse(slips = [],
 slip_count = 0,
 leg_count = leg_count,
 platform = platform.lower(),
 total_candidates = 0,
...
async def quote_parlay(request: "QuoteRequest",
 db: AsyncSession = Depends(get_db),):
 """
 ...
 result = await quote_parlay_legs(db,
 legs = legs_data,
...
async def get_odds_health(sport_id: Optional[int] = Query(None, description = "Optional sport ID filter"),
 db: AsyncSession = Depends(get_db),):
 """
...
async def explore_alt_lines(pick_id: int,
 db: AsyncSession = Depends(get_db),):
 """
...
async def get_sport_freshness(sport_key: str,
 db: AsyncSession = Depends(get_db),):
 """
...
async def debug_date_check(db: AsyncSession = Depends(get_db)):
 ...
 today_games = await db.scalar(select(func.count(Game.id)).where(Game.start_time > = today_utc,
 Game.start_time < tomorrow_utc,))

 # Get a sample of recent games
 recent_games_result = await db.execute(select(Game.external_game_id, Game.start_time)
 ...
 "diagnosis": ("STALE DATA: No games match today's filter. Run force - refresh."
