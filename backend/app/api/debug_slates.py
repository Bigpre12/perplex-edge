...
@router.get(" / slates")
async def debug_slates(sport_id: int = Query(30, description = "Sport ID"),
 ...
 upcoming_games_query = select(func.count(Game.id)).where(and_(Game.sport_id =  = sport_id,
 Game.start_time > = now,
 ...
 picks_with_time_query = select(func.count(ModelPick.id)).where(ModelPick.generated_at.isnot(None))
 ...
 recent_picks_query = select(func.count(ModelPick.id)).where(ModelPick.generated_at > = (datetime.now(timezone.utc) - timedelta(hours = 24)))
 ...
 # Get sample data
 try:
 sample_picks_query = select(ModelPick).options(selectinload(ModelPick.player).selectinload(Player.team),
 selectinload(ModelPick.market)).where(ModelPick.sport_id =  = sport_id).limit(3)
