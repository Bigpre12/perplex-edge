...
async def validate_model_calibration(sport_id: int = Path(description = "Sport ID to validate"),
...
async def calculate_clv_performance(sport_id: int = Path(description = "Sport ID to analyze"),
...
async def get_model_status_report(sport_id: int = Path(description = "Sport ID to analyze"),
 db: AsyncSession = Depends(get_db)):
