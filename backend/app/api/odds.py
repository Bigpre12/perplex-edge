...
async def list_lines(game_id: Optional[int] = Query(None, description = "Filter by game"),
...
async def compare_lines(game_id: int,
 market_type: Optional[str] = Query(None, description = "Filter by market type"),
 db: AsyncSession = Depends(get_db),):
 ...
 for line in lines:
 market = line.market
 key = (market.market_type if market else "unknown",
...
async def get_best_lines(sport: Optional[str] = Query(None, description = "Filter by sport"),
 ...
 query = (select(Line)
 .options(selectinload(Line.market),
 ...
 bookmaker_lines = [
 BookmakerLine(sportsbook = l.sportsbook,
 odds = l.odds,
