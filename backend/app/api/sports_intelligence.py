...
@router.get(" / news")
async def get_sports_news(limit: int = Query(default = 20, le = 100),
...
@router.get(" / weather")
async def get_weather_conditions(condition: str = Query(None, description = "Filter by condition: clear, rain, snow, windy, extreme"),
...
async def get_intelligence_recommendations(recommendation: str = Query(None, description = "Filter by recommendation: over, under, pass, avoid"),
