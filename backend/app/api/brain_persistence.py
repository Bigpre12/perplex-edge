...
from app.services.brain_persistence_integration import (
    get_brain_analytics,
...
async def get_brain_decisions(
    ...
    hours: int = Query(24, ge=1, le=168)  # 1 week max
) -> Dict[str, Any]:
...
async def log_manual_decision(
    ...
    try:
        decision_id = await brain_persistence.log_decision(
            category=category,
            action=action,
            reasoning=reasoning,
            outcome=outcome,
...
async def log_manual_health_check(
    ...
    try:
        check_id = await brain_persistence.log_health_check(
            component=component,
            status=status,
            message=message,
            details=details,
