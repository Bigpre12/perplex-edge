...
@router.get(" / ")
async def get_parlays(sport_id: int = Query(None, description = "Filter by sport ID"),
 ...
 query = select(ModelPick).options(selectinload(ModelPick.player).selectinload(Player.team).selectinload(Team.sport),
 ...
 conditions.append(ModelPick.game_id.in_(select(Game.id).where(Game.status.in_(['scheduled', 'in_progress']))))

 query = query.where(and_( * conditions)).order_by(desc(ModelPick.expected_value)).limit(limit * 2) # Get extra for diversity
...
@router.get(" / build")
async def build_parlays(sport_id: int = Query(None, description = "Filter by sport ID"),
 ...
 query = query.where(and_( * conditions)).order_by(desc(ModelPick.expected_value)).limit(limit * 3) # Get extra candidates for diversity
...
@router.get(" / sports")
async def get_sports_parlays(sport_id: int = Query(..., description = "Sport ID(required)"),
 min_ev: float = Query(default = 0.05, description = "Minimum expected value"),
 min_confidence: float = Query(default = 0.6, description = "Minimum confidence score"),
 limit: int = Query(default = 10, le = 20),
 db: AsyncSession = Depends(get_db)):
...
async def get_player_props_parlays(player_id: int = Query(None, description = "Player ID(optional)"),
 ...
 query = query.where(and_( * conditions)).order_by(desc(ModelPick.expected_value)).limit(limit * 2)
