...
async def sync_user(request: UserSyncRequest,
 ...
 result = await db.execute(select(User).where(User.id =  = request.clerk_id))
...
async def increment_props_viewed(user_id: str,
 db: AsyncSession = Depends(get_db),):
 """
...
async def get_whop_checkout_urls():
 ...
 return WhopCheckoutResponse(free_checkout_url = settings.whop_free_checkout_url,
