...
async def get_calibration_metrics(sport_id: int,
...
async def train_calibration_model(sport_id: int,
...
async def calibrate_probability(model_prob: float = Query(..., description = "Model probability(0 - 1)"),
 ...
 try:
 calibrator = ProbabilityCalibrator()
 calibrated_prob = calibrator.calibrate_probability(model_prob, implied_prob, sample_size, market_type)

 confidence_interval = calibrator.calculate_confidence_interval(calibrated_prob, sample_size)
