...
from app.schemas.trade import(TradeCreate, TradeRead, TradeList, TradeWithDetails,
...
async def get_team_by_name(db: AsyncSession, team_name: str, sport_id: int = 30) - > Optional[Team]:
 ...
 result = await db.execute(select(Team).where(and_(Team.sport_id =  = sport_id,
 or_(Team.name.ilike(f"%{team_name}%"),
...
async def get_player_by_name(db: AsyncSession, player_name: str, sport_id: int = 30) - > Optional[Player]:
 ...
 result = await db.execute(select(Player).where(and_(Player.sport_id =  = sport_id,
...
def build_trade_response(trade: Trade) - > TradeWithDetails:
 ...
 details_with_teams.append(TradeDetailWithTeams(id = detail.id,
 ...
 return TradeWithDetails(id = trade.id,
...
async def list_trades(season_year: Optional[int] = Query(None, description = "Filter by season year(e.g., 2026)"),
 ...
 """
 query = (select(Trade)
 .options(selectinload(Trade.details).selectinload(TradeDetail.from_team),
 ...
 if team_obj:
 query = query.join(Trade.details).where(or_(TradeDetail.from_team_id =  = team_obj.id,
 TradeDetail.to_team_id =  = team_obj.id,)).distinct()

 if player:
 query = query.join(Trade.details).where(TradeDetail.player_name.ilike(f"%{player}%")).distinct()
 ...
 return TradeList(items = [build_trade_response(t) for t in trades],
 total = total,)
...
async def get_player_trade_history(player_id: int,
 db: AsyncSession = Depends(get_db),):
 ...
 selectinload(Trade.details).selectinload(TradeDetail.player),)
 .join(Trade.details)
 .where(or_(TradeDetail.player_id =  = player_id,
...
async def create_trade(trade_data: TradeCreate,
 ...
 """
 # Create the trade
 trade = Trade(trade_date = trade_data.trade_date,
 ...
 detail = TradeDetail(trade_id = trade.id,
 player_id = player_id,
...
async def bulk_create_trades(bulk_data: BulkTradeCreate,
 db: AsyncSession = Depends(get_db),):
 """
 ...
 try:
 # Create trade
 trade = Trade(trade_date = trade_item.trade_date,
 ...
 return BulkTradeResult(trades_created = trades_created,
...
async def _apply_trade_internal(db: AsyncSession, trade: Trade) - > dict:
 ...
 season_result = await db.execute(select(Season).where(and_(Season.sport_id =  = player.sport_id,
 ...
 if old_team_id:
 old_roster_result = await db.execute(select(SeasonRoster).where(and_(SeasonRoster.season_id =  = season.id,
 ...
 new_roster = SeasonRoster(season_id = season.id,
...
async def apply_trade(trade_id: int,
 ...
 logger.info(f"Applied trade {trade_id}: {apply_result['players_updated']} players updated")

 return TradeApplyResult(trade_id = trade_id,
 players_updated = apply_result["players_updated"],
