...
async def list_injuries(sport: Optional[str] = Query(None, description = "Filter by sport"),
 ...
 query = (select(Injury)
 .options(selectinload(Injury.player).selectinload(Player.team),)
 ...
 items.append(InjuryWithPlayer(id = injury.id,
...
async def get_injuries_by_game(game_id: int,
 db: AsyncSession = Depends(get_db),):
