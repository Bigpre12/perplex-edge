...
async def debug_model_picks_count(sport_id: int = Query(30),
 ...
 recent_picks_result = await db.execute(select(func.count(ModelPick.id))
 .join(Game, ModelPick.game_id =  = Game.id)
 ...
 day_picks_result = await db.execute(select(func.count(ModelPick.id))
 .join(Game, ModelPick.game_id =  = Game.id)
 .where(and_(Game.sport_id =  = sport_id,
 ...
 three_day_picks_result = await db.execute(select(func.count(ModelPick.id))
 .join(Game, ModelPick.game_id =  = Game.id)
 .where(and_(Game.sport_id =  = sport_id,
 ...
 sample_result = await db.execute(select(ModelPick, Player, Game)
...
async def debug_parlay_query(sport_id: int = Query(30),
 db: AsyncSession = Depends(get_db)):
 ...
 # Run the exact query
 result = await db.execute(select(ModelPick, Player, Game, Team, Market)
 .join(Player, ModelPick.player_id =  = Player.id)
 .join(Game, ModelPick.game_id =  = Game.id)
