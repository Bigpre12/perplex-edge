...
from app.services.nfl_backup import(list_backups,
 backup_exists,
 delete_old_backups,)
...
@router.get(" / odds")
async def get_nfl_live_odds(week: Optional[int] = Query(None, ge = 1, le = 22, description = "NFL week(1 - 18 regular, 19 - 22 playoffs)"),
...
async def sync_nfl_games(include_props: bool = Query(True, description = "Include player props"),
...
async def get_nfl_historical_odds(week: Optional[int] = Query(None, ge = 1, le = 22, description = "NFL week filter"),
...
async def get_hit_rates(season: Optional[int] = Query(None, ge = 2020, le = 2030, description = "Season year"),
 bookmaker: Optional[str] = Query(None, description = "Filter by bookmaker"),
 db: AsyncSession = Depends(get_db),):
 """
...
@router.post(" / setup")
async def setup_nfl(db: AsyncSession = Depends(get_db),):
 """
