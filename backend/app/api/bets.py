...
from app.services.bets_service import (
    create_bet,
...
async def list_sportsbooks(db: AsyncSession = Depends(get_db)):
    ...
    result = await db.execute(
        select(distinct(UserBet.sportsbook)).order_by(UserBet.sportsbook)
    )
...
async def list_market_types(db: AsyncSession = Depends(get_db)):
    ...
    result = await db.execute(
        select(distinct(UserBet.market_type)).order_by(UserBet.market_type)
    )
