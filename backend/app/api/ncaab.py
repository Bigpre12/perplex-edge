...
from app.services.ncaab_backup import(list_backups,
 backup_exists,
 delete_old_backups,)
...
@router.get(" / odds")
async def get_ncaab_live_odds(season: Optional[str] = Query(None, description = "Season filter(e.g., '2025 - 2026')"),
...
async def sync_ncaab_games(include_props: bool = Query(True, description = "Include player props"),
 ...
 picks_result = None
 if generate_picks:
 picks_result = await gen_picks(db,
...
async def get_ncaab_historical_odds(season: Optional[str] = Query(None, description = "Season filter(e.g., '2025 - 2026')"),
 bookmaker: Optional[str] = Query(None, description = "Filter by bookmaker"),
 ...
 """
 try:
 odds = await get_historical_odds(db,
 season = season,
 bookmaker = bookmaker,
...
async def create_snapshot(snapshot_date: Optional[date] = Query(None, description = "Snapshot date(defaults to today)"),
 db: AsyncSession = Depends(get_db),):
 """
...
async def set_game_result(game_id: str,
 result: str = Query(..., pattern = "^(home|away)$", description = "Game result"),
 db: AsyncSession = Depends(get_db),):
 """
...
async def get_hit_rates(season: Optional[str] = Query(None, description = "Season filter(e.g., '2025 - 2026')"),
 bookmaker: Optional[str] = Query(None, description = "Filter by bookmaker"),
 db: AsyncSession = Depends(get_db),):
 """
...
@router.get(" / backups")
async def get_backups(limit: int = Query(30, ge = 1, le = 100, description = "Maximum number of backups to list"),):
 """
...
async def cleanup_old_backups(days_to_keep: int = Query(90, ge = 7, le = 365, description = "Days of backups to retain"),):
 """
...
@router.post(" / setup")
async def setup_ncaab(db: AsyncSession = Depends(get_db),):
 """
