...
from app.services import(sync_games_and_lines,
...
async def run_quota_safe_sync(sport: str = Query("basketball_nba", description = "Sport key to sync"),
 ...
 try:
 result = await sync_with_fallback(db,
 sport_key = sport,
...
async def browser_force_refresh(sport: str,
 db: AsyncSession = Depends(get_db),):
 """
 ...
 # Sync fresh data
 sync_result = await sync_with_fallback(db,
 sport_key = sport,
 include_props = True,
 use_real_api = False,)

 # Generate picks for the synced data
 picks_result = await generate_picks(db,
 sport_key = sport,
 min_ev = 0.0,
 min_confidence = 0.5,
 use_stubs = True,)
...
async def manual_refresh_live(sport: str,
 db: AsyncSession = Depends(get_db),):
 """
...
async def force_refresh_games(sport: str = Query("basketball_nba", description = "Sport key to refresh"),
 db: AsyncSession = Depends(get_db),):
 """
...
async def test_discord_alert():
 ...
 # Send test message
 embed = create_embed(title = "Test Alert",
...
async def test_high_ev_alert(db: AsyncSession = Depends(get_db),):
 """
 ...
 # Send sample alert
 success = await alert_high_ev_pick(player_name = "LeBron James",
 stat_type = "PTS",
 line = 24.5,
 side = "over",
 odds = - 115,
...
async def get_snapshots(sport: Optional[str] = Query(None, description = "Filter by sport key"),
...
async def create_snapshot(sport: str = Query("basketball_nba", description = "Sport to snapshot"),
 db: AsyncSession = Depends(get_db),):
 """
...
async def get_snapshot(sport: str,
 snapshot_date: str,):
 """
...
async def fix_model_picks_sport_mappings(db: AsyncSession = Depends(get_db)):
 ...
 incorrect_picks_result = await db.execute(select(ModelPick, Player)
 ...
 await db.execute(update(ModelPick)
...
async def verify_model_picks_sport_mappings(db: AsyncSession = Depends(get_db)):
 ...
 nhl_picks_in_nhl_result = await db.execute(select(func.count(ModelPick.id))
 .join(Player, ModelPick.player_id =  = Player.id)
 .where(Player.sport_id =  = 53, ModelPick.sport_id =  = 53))
 verification_results["nhl_picks_in_nhl"] = nhl_picks_in_nhl_result.scalar() or 0

 nhl_picks_in_nba_result = await db.execute(select(func.count(ModelPick.id))
 .join(Player, ModelPick.player_id =  = Player.id)
...
async def fix_sport_mappings_production(db: AsyncSession = Depends(get_db)):
 ...
 # Get NBA teams
 nba_teams_result = await db.execute(select(Team).where(Team.sport_id =  = 30) # NBA sport_id)
 ...
 nba_players_in_nhl_result = await db.execute(select(Player)
 .where(Player.team_id.in_(nba_team_ids),
 ...
 await db.execute(update(Player)
 ...
 # Get NHL teams
 nhl_teams_result = await db.execute(select(Team).where(Team.sport_id =  = 53) # NHL sport_id)
 ...
 nhl_players_in_nba_result = await db.execute(select(Player)
 .where(Player.team_id.in_(nhl_team_ids),
 ...
 incorrect_mappings_result = await db.execute(select(Player, Team)
 ...
 picks_to_fix_result = await db.execute(select(ModelPick, Game)
 ...
 # Calculate totals
 changes_made["total_players_fixed"] = (changes_made["nba_players_fixed"] + 
...
async def verify_sport_mappings_production(db: AsyncSession = Depends(get_db)):
 ...
 try:
 # Count NBA players
 nba_teams_result = await db.execute(select(Team).where(Team.sport_id =  = 30))
 nba_teams = nba_teams_result.scalars().all()
 nba_team_ids = [team.id for team in nba_teams]

 if nba_team_ids:
 # NBA players correctly in NBA
 nba_players_in_nba_result = await db.execute(select(func.count(Player.id))
 .where(Player.team_id.in_(nba_team_ids),
 Player.sport_id =  = 30))
 verification_results["nba_players_in_nba"] = nba_players_in_nba_result.scalar() or 0

 # NBA players incorrectly in NHL
 nba_players_in_nhl_result = await db.execute(select(func.count(Player.id))
 .where(Player.team_id.in_(nba_team_ids),
 Player.sport_id =  = 53))
 verification_results["nba_players_in_nhl"] = nba_players_in_nhl_result.scalar() or 0

 # Count NHL players
 nhl_teams_result = await db.execute(select(Team).where(Team.sport_id =  = 53))
 nhl_teams = nhl_teams_result.scalars().all()
 nhl_team_ids = [team.id for team in nhl_teams]

 if nhl_team_ids:
 # NHL players correctly in NHL
 nhl_players_in_nhl_result = await db.execute(select(func.count(Player.id))
 .where(Player.team_id.in_(nhl_team_ids),
 Player.sport_id =  = 53))
 verification_results["nhl_players_in_nhl"] = nhl_players_in_nhl_result.scalar() or 0

 # NHL players incorrectly in NBA
 nhl_players_in_nba_result = await db.execute(select(func.count(Player.id))
 .where(Player.team_id.in_(nhl_team_ids),
 Player.sport_id =  = 30))
 ...
 incorrect_mappings_result = await db.execute(select(func.count(Player.id))
 .select_from(select(Player.id)
 .join(Team, Player.team_id =  = Team.id)
 .where(Player.sport_id ! = Team.sport_id)
 .subquery()))
 verification_results["incorrect_mappings"] = incorrect_mappings_result.scalar() or 0
...
async def get_stale_data_alerts(max_age_hours: int = Query(24, description = "Hours before data is considered stale"),
 db: AsyncSession = Depends(get_db),):
 """
...
async def verify_sport_id_counts(db: AsyncSession = Depends(get_db),):
 """
 ...
 # Count games
 games_count = await db.scalar(select(func.count()).select_from(Game).where(Game.sport_id =  = sport_id)) or 0

 # Count lines(props)
 lines_count = await db.scalar(select(func.count()).select_from(Line)
 .join(Game, Line.game_id =  = Game.id)
 .where(Game.sport_id =  = sport_id)) or 0

 # Count player props(lines with player_id)
 props_count = await db.scalar(select(func.count()).select_from(Line)
 .join(Game, Line.game_id =  = Game.id)
 .where(Game.sport_id =  = sport_id, Line.player_id.isnot(None))) or 0

 # Count picks
 picks_count = await db.scalar(select(func.count()).select_from(ModelPick).where(ModelPick.sport_id =  = sport_id)) or 0
...
async def verify_sportsbooks(db: AsyncSession = Depends(get_db),):
 """
 ...
 sportsbook_result = await db.execute(select(Line.sportsbook,
 ...
 sport_books_result = await db.execute(select(Game.sport_id,
 Line.sportsbook,
...
async def verify_players_by_sport(db: AsyncSession = Depends(get_db),):
 """
 ...
 # Get sample players
 players_result = await db.execute(select(Player.id, Player.name, Player.external_player_id)
...
async def debug_data_freshness(db: AsyncSession = Depends(get_db),):
 """
 ...
 # Get sync metadata
 sync_result = await db.execute(select(SyncMetadata)
 .where(and_(SyncMetadata.sport_key =  = sport_key,
 ...
 today_games = await db.scalar(select(func.count())
 .select_from(Game)
 .where(and_(Game.sport_id =  = sport_id,
 Game.start_time > = today_start,
 Game.start_time < tomorrow,))) or 0

 upcoming_games = await db.scalar(select(func.count())
 .select_from(Game)
 .where(and_(Game.sport_id =  = sport_id,
 Game.start_time > = now.replace(tzinfo = None),
 Game.start_time < (now + timedelta(days = 7)).replace(tzinfo = None),))) or 0

 # Count props / lines
 props_count = await db.scalar(select(func.count())
 .select_from(Line)
 .join(Game, Line.game_id =  = Game.id)
 .where(and_(Game.sport_id =  = sport_id,
 Line.is_current =  = True,
 Line.player_id.isnot(None),))) or 0

 # Count picks
 picks_count = await db.scalar(select(func.count())
 .select_from(ModelPick)
 .where(and_(ModelPick.sport_id =  = sport_id,
 ModelPick.is_active =  = True,))) or 0
 ...
 "alerts": _get_sport_alerts(sport_key, sport_status, today_games, props_count, picks_count, staleness),
 })
...
def _get_sport_alerts(sport_key: str,
 sport_status: dict,
 today_games: int,
 props_count: int,
 picks_count: int,
 staleness: str,) - > list[str]:
...
async def get_api_metrics(minutes: int = Query(60, description = "Time window in minutes"),):
 """
...
async def get_metrics_dashboard(db: AsyncSession = Depends(get_db),):
 ...
 picks_today_result = await db.execute(select(func.count(ModelPick.id)).where(ModelPick.generated_at > = today_start))
 picks_today = picks_today_result.scalar() or 0

 # Count total active picks
 total_picks_result = await db.execute(select(func.count(ModelPick.id)).where(ModelPick.is_active =  = True))
 ...
 "health_summary": {
 "all_breakers_closed": all(info.get("state") ! = "open"
 for info in circuit_status.values()) if circuit_status else True,
...
async def get_data_quality(sport: str,
 db: AsyncSession = Depends(get_db),):
 """
...
@router.get(" / quality")
async def get_all_data_quality(db: AsyncSession = Depends(get_db),):
 """
...
async def get_calibration_report(sport: str,
...
async def get_reliability_plot_data(sport: str,
 days: int = Query(30, description = "Number of days to analyze"),
 db: AsyncSession = Depends(get_db),):
 """
...
async def get_roi_by_confidence_bucket(sport: str,
 days: int = Query(30, description = "Number of days to analyze"),
 db: AsyncSession = Depends(get_db),):
 """
...
async def get_clv_analysis_endpoint(sport: str,
 days: int = Query(30, description = "Number of days to analyze"),
 db: AsyncSession = Depends(get_db),):
 """
...
async def run_sync_odds_job(sport: str = Query("basketball_nba", description = "Sport key to sync"),
 ...
 try:
 result = await sync_games_and_lines(db,
 sport_key = sport,
 include_props = include_props,
 use_stubs = use_stubs,)
...
async def run_sync_stats_job(sport: str = Query("basketball_nba", description = "Sport key to sync"),
 ...
 try:
 result = await sync_recent_player_stats(db,
 sport_key = sport,
...
async def run_sync_injuries_job(sport: str = Query("basketball_nba", description = "Sport key to sync"),
...
async def sync_injuries_from_espn(sport: str = Query("basketball_nba", description = "Sport key to sync"),
 db: AsyncSession = Depends(get_db),):
 """
...
async def run_generate_picks_job(sport: str = Query("basketball_nba", description = "Sport key to generate picks for"),
 ...
 try:
 result = await generate_picks(db,
 sport_key = sport,
 min_ev = min_ev,
...
async def run_all_jobs(sport: str = Query("basketball_nba", description = "Sport key to process"),
 use_stubs: bool = Query(False, description = "Use stub data for testing"),
 db: AsyncSession = Depends(get_db),):
 """
...
@router.post(" / cache / clear")
async def clear_cache(key_prefix: Optional[str] = Query(None, description = "Clear only keys with this prefix"),):
 """
 Clear the in - memory cache.
...
async def get_dashboard_summary(db: AsyncSession = Depends(get_db),):
 ...
 game_rows = await db.execute(select(Game.sport_id, func.count(Game.id))
 .where(and_(Game.sport_id.in_(sport_ids),
 ...
 line_rows = await db.execute(select(Game.sport_id,
 ...
 logger.error("dashboard_summary_error",
 error = str(e),
