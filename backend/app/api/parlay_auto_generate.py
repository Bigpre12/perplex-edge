...
async def auto_generate_parlays(platform: str = Query(default = "prizepicks", description = "Platform"),
 ...
 query = query.where(and_( * conditions)).order_by(desc(ModelPick.expected_value)).limit(slip_count * leg_count * 2) # Get extra candidates
 ...
 if(pick.player_id not in current_players and(pick.player_id, pick.market_id) not in current_stats):
