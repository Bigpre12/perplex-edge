...
async def list_picks(sport: Optional[str] = Query(None, description = "Filter by sport"),
 ...
 query = (select(ModelPick)
 .options(selectinload(ModelPick.sport),
 ...
 items.append(ModelPickWithDetails(id = pick.id,
...
async def get_top_picks(sport: Optional[str] = Query(None, description = "Filter by sport"),
...
async def get_picks_summary(sport: Optional[str] = Query(None, description = "Filter by sport"),
 db: AsyncSession = Depends(get_db),):
 ...
 # Active picks
 active_result = await db.execute(select(func.count()).select_from(base_query.where(ModelPick.is_active =  = True).subquery()))
 active_picks = active_result.scalar() or 0

 # Average EV
 avg_ev_result = await db.execute(select(func.avg(ModelPick.expected_value)).select_from(base_query.where(ModelPick.is_active =  = True).subquery()))
 avg_ev = avg_ev_result.scalar() or 0.0

 # Average confidence
 avg_conf_result = await db.execute(select(func.avg(ModelPick.confidence_score)).select_from(base_query.where(ModelPick.is_active =  = True).subquery()))
 avg_confidence = avg_conf_result.scalar() or 0.0

 # High confidence picks(> 0.7)
 high_conf_result = await db.execute(select(func.count()).select_from(base_query.where(ModelPick.is_active =  = True,
 ModelPick.confidence_score > 0.7,).subquery()))
 high_confidence_picks = high_conf_result.scalar() or 0

 return PickSummary(total_picks = total_picks,
...
@router.post(" / refresh")
async def refresh_picks(sport: str = Query("nba", description = "Sport to refresh: nba, nfl, mlb, nhl"),
 db: AsyncSession = Depends(get_db),):
 """
 ...
 try:
 sync_stats = await sync_games_and_lines(db, sport_key, include_props = True, use_stubs = False,
 ...
 sync_stats = await sync_games_and_lines(db, sport_key, include_props = True, use_stubs = True)
 data_source = "stubs"
