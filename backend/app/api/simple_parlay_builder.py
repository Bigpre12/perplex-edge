...
@router.get(" / ")
async def get_simple_parlays(sport_id: int = Query(None, description = "Filter by sport ID"),
...
async def get_available_picks(sport_id: int = Query(None, description = "Filter by sport ID"),
 ...
 query = query.where(and_( * conditions)).order_by(desc(ModelPick.expected_value)).limit(limit)
...
@router.get(" / build")
async def build_simple_parlays(sport_id: int = Query(None, description = "Filter by sport ID"),
 ...
 query = query.where(and_( * conditions)).order_by(desc(ModelPick.expected_value)).limit(limit * 3)
