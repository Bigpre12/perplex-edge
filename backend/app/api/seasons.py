...
from app.models import(Season, SeasonRoster, Sport, Team, Player, Game, PlayerGameStats,)
from app.schemas.season import(SeasonRead, SeasonList,
...
async def list_seasons(sport_id: int = Query(..., description = "Sport ID(e.g., 30 = NBA, 31 = NFL)"),
 ...
 return SeasonList(items = [SeasonRead.model_validate(s) for s in seasons],
 total = len(seasons),)

@router.get(" / seasons / current", response_model = Optional[SeasonRead], tags = ["seasons"])
async def get_current_season(sport_id: int = Query(..., description = "Sport ID"),
 db: AsyncSession = Depends(get_db),):
 """
 ...
 """
 query = (select(Season)
 .where(and_(Season.sport_id =  = sport_id,
...
async def list_teams_for_season(sport_id: int,
 ...
 .where(and_(Team.sport_id =  = sport_id,
...
@router.get(" / sports / {sport_id} / teams / {team_id} / roster",
 response_model = List[RosterPlayerOut],
 tags = ["seasons"],)
async def list_team_roster(sport_id: int,
 team_id: int,
 ...
 query = (select(SeasonRoster, Player)
 .join(Season, Season.id =  = SeasonRoster.season_id)
...
@router.get(" / sports / {sport_id} / games",
 response_model = List[GameOut],
 tags = ["seasons"],)
async def list_games_for_date(sport_id: int,
...
@router.get(" / games / {game_id} / stats",
 ...
 tags = ["seasons"],)
async def list_game_player_stats(game_id: int,
 db: AsyncSession = Depends(get_db),):
 """
 ...
 query = (select(PlayerGameStats, Player, Team)
 ...
 stats = data["stats"]
 results.append(GamePlayerOut(player = PlayerInfo(id = data["player"].id,
 sport_id = data["player"].sport_id,
 team_id = data["player"].team_id,
 name = data["player"].name,
 position = data["player"].position,),
 team = TeamInfo(id = data["team"].id,
 sport_id = data["team"].sport_id,
 name = data["team"].name,
 abbreviation = data["team"].abbreviation,),
 stats = GamePlayerStats(minutes = stats.get("minutes"),
...
@router.get(" / sports / {sport_id} / players",
 response_model = List[PlayerInfo],
 tags = ["seasons"],)
async def list_players_for_sport(sport_id: int,
