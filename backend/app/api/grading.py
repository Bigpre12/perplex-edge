...
async def grade_completed_picks(background_tasks: BackgroundTasks,
...
async def get_grading_statistics(days_back: int = Query(default = 30, description = "Days to look back"),
 db: AsyncSession = Depends(get_db)):
...
async def backtest_month(year: int,
 month: int,
...
async def get_model_status(db: AsyncSession = Depends(get_db)):
 ...
 backtest_results = await backtester.backtest_month(last_month.year, last_month.month)
...
async def debug_picks_status(db: AsyncSession = Depends(get_db)):
 ...
 try:
 result = await db.execute(select(func.count(ModelPick.id).label('total'),
 func.count(ModelPick.id).filter(ModelPick.sport_id =  = 30).label('nba_total'),
 func.count(ModelPick.id).filter((ModelPick.sport_id =  = 30) & (ModelPick.is_active =  = True)).label('nba_active'),
 func.count(ModelPick.id).filter((ModelPick.sport_id =  = 30) & (ModelPick.expected_value > 0.15)).label('nba_high_ev'),
 func.avg(ModelPick.expected_value).filter(ModelPick.sport_id =  = 30).label('nba_avg_ev')))
...
async def activate_test_picks(count: int = 100,
 db: AsyncSession = Depends(get_db)):
 """
 ...
 result = await db.execute(select(ModelPick.id)
 ...
 .values(expected_value = 0.03, # 3% EV(realistic)
 is_active = True))
...
async def delete_bad_picks(db: AsyncSession = Depends(get_db)):
 ...
 try:
 result = await db.execute(select(func.count(ModelPick.id))
 .where(ModelPick.expected_value > 0.15))
 count_before = result.scalar()

 # Delete them
 await db.execute(text("DELETE FROM model_picks WHERE expected_value > 0.15"))
 await db.commit()
