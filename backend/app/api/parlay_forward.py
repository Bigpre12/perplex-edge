...
@router.get(" / ")
async def get_parlays_forward(sport_id: int = Query(None, description = "Filter by sport ID"),
...
@router.get(" / build")
async def build_parlays_forward(sport_id: int = Query(None, description = "Filter by sport ID"),
...
@router.get(" / sports")
async def get_sports_parlays_forward(sport_id: int = Query(..., description = "Sport ID(required)"),
 min_ev: float = Query(default = 0.05, description = "Minimum expected value"),
 min_confidence: float = Query(default = 0.6, description = "Minimum confidence score"),
 limit: int = Query(default = 10, le = 20),
 db: AsyncSession = Depends(get_db)):
 ...
 result = await get_ultra_simple_parlays(sport_id = sport_id,
 limit = limit,
 db = db)
...
async def get_player_props_parlays_forward(player_id: int = Query(None, description = "Player ID(optional)"),
 ...
 # Call the working ultra - simple parlay builder
 result = await get_available_picks(sport_id = sport_id,
 limit = limit,
 db = db)
...
async def get_available_picks_forward(sport_id: int = Query(None, description = "Filter by sport ID"),
