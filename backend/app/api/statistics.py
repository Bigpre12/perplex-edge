...
async def get_player_statistics(player_id: int,
 ...
 .options(selectinload(Player.team).selectinload(Team.sport))
 ...
 picks_result = await db.execute(select(ModelPick)
...
async def get_player_career_stats(player_id: int, db: AsyncSession = Depends(get_db)):
 ...
 stats_result = await db.execute(select(func.count(PlayerStat.season).label('seasons_played'),
 ...
 best_result = await db.execute(select(PlayerStat)
 .where(PlayerStat.player_id =  = player_id)
...
async def get_team_statistics(team_id: int,
 ...
 players_stats.sort(key = lambda x: x.get("points_per_game", 0),
 reverse = True)
...
async def get_league_leaders(category: str = Query(default = "points_per_game", description = "Statistical category"),
 season: int = Query(None, description = "Filter by season"),
...
async def get_season_summary(season: int, db: AsyncSession = Depends(get_db)):
 ...
 .where(and_(func.extract('year', Game.start_time) =  = season,
 Game.status =  = 'completed')))
 summary["total_games"] = games_result.scalar() or 0

 # Get total players with stats
 players_result = await db.execute(select(func.count(func.distinct(PlayerStat.player_id)))
 .where(PlayerStat.season =  = season))
 summary["total_players"] = players_result.scalar() or 0

 # Get league averages
 averages_result = await db.execute(select(func.avg(PlayerStat.points_per_game).label('avg_points'),
 func.avg(PlayerStat.rebounds_per_game).label('avg_rebounds'),
 func.avg(PlayerStat.assists_per_game).label('avg_assists'),
 func.avg(PlayerStat.field_goal_percentage).label('avg_fg_pct'))
 .where(PlayerStat.season =  = season))
 ...
 leaders_result = await db.execute(select(PlayerStat, Player)
 .join(Player, PlayerStat.player_id =  = Player.id)
 .options(selectinload(Player.team))
 .where(and_(PlayerStat.season =  = season,
