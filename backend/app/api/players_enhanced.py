...
@router.get(" / ")
async def get_players(sport_id: int = Query(None, description = "Filter by sport ID"),
 ...
 query = select(Player).options(selectinload(Player.team).selectinload(Team.sport),
...
async def get_player(player_id: int, db: AsyncSession = Depends(get_db)):
 ...
 stats_result = await db.execute(select(PlayerStat).where(PlayerStat.player_id =  = player_id)
...
async def get_team_players(team_id: int,
 ...
 try:
 query = select(Player).options(selectinload(Player.team).selectinload(Team.sport)).where(Player.team_id =  = team_id)
...
async def get_sport_players(sport_id: int,
 position: str = Query(None, description = "Filter by position"),
 ...
 try:
 query = select(Player).options(selectinload(Player.team).selectinload(Team.sport)).where(Player.team.has(Team.sport_id =  = sport_id))
