...
def verify_whop_signature(payload: bytes, signature: str) - > bool:
 ...
 expected = hmac.new(WHOP_WEBHOOK_SECRET.encode(),
 payload,
...
async def handle_membership_ended(data: dict, db: AsyncSession):
 ...
 result = await db.execute(select(User).where(User.whop_membership_id =  = membership_id))
 user = result.scalar_one_or_none()
...
async def test_upgrade_user(user_id: str,
 plan: str = "pro",
 db: AsyncSession = Depends(get_db),):
 """
