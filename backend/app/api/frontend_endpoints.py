...
async function getParlays(sportId = 30, legCount = 3) {
 const response = await fetch(`${baseUrl} / api / multisport / sports / ${sportId} / parlays / builder?leg_count = ${legCount}&min_leg_grade = C&max_results = 5`);
 const data = await response.json();
 return data.parlays;
}

async function getMarketAnalysis(sportId = 30) {
 const response = await fetch(`${baseUrl} / api / sportsbook / market - analysis?sport_id = ${sportId}`);
...
def get_parlays(sport_id = 30, leg_count = 3):
 response = requests.get(f'{base_url} / api / multisport / sports / {sport_id} / parlays / builder',
 params = {'leg_count': leg_count, 'min_leg_grade': 'C', 'max_results': 5})
 return response.json()['parlays']

def get_market_analysis(sport_id = 30):
 response = requests.get(f'{base_url} / api / sportsbook / market - analysis',
