...
from app.services.stats_calculator import(calculate_real_hit_rates,
...
async def get_player_hit_rates(player_id: int,
 ...
 """
 try:
 result = await calculate_real_hit_rates(db,
 player_id = player_id,
 stat_type = stat_type,
 games = games,)
 return result
...
async def get_ev_performance(player_id: Optional[int] = Query(None, description = "Optional player filter"),
 ...
 """
 try:
 result = await calculate_ev_performance(db,
 player_id = player_id,
 days = days,)
 return result
 except Exception as e:
...
async def get_odds_trends(player_id: int,
 ...
 """
 try:
 result = await calculate_odds_trends(db,
 player_id = player_id,
 stat_type = stat_type,
 days = days,)
 return result
...
@router.get(" / backtest")
async def run_backtest(sport_id: Optional[int] = Query(None, description = "Filter by sport"),
...
async def get_market_performance(days: int = Query(30, ge = 1, le = 365, description = "Days of history to analyze"),
 ...
 settled_filter = and_(base_filter,
 UserBet.status.cast(String) ! = BetStatus.PENDING.value,)

 result = await db.execute(select(UserBet.market_type,
 ...
 logger.error("market_performance_error",
 error = str(e),
...
async def get_dashboard(sport: str = Query("nba", description = "Sport: nba, ncaab, nfl, ncaaf, mlb, nhl, atp, wta"),
 ...
 try:
 result = await get_analytics_dashboard(db,
 sport_key = sport_key,
 days = days,)
 return result
 except Exception as e:
...
@router.post(" / sync")
async def trigger_historical_sync(sport: str = Query("nba", description = "Sport to sync"),
 db: AsyncSession = Depends(get_db),):
 """
...
async def trigger_historical_odds_sync(sport: str = Query("nba", description = "Sport to sync"),
...
async def trigger_results_sync(sport: str = Query("nba", description = "Sport to sync"),
