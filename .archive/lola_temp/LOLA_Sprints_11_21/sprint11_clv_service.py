...
def calculate_clv(bet: BetLog, closing_line: float) -> dict:
...
def settle_clv_for_user(user_id: str, db: Session):
    ...
    for bet in bets:
        ...
        if not closing:
            continue
        result = calculate_clv(bet, closing.line)
        bet.clv = result['clv']
...
def get_clv_summary(user_id: str, db: Session) -> dict:
