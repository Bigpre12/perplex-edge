...
scanner_router       = safe_import("scanner", "router")
history_router       = safe_import("history", "router")
elite_router         = safe_import("elite", "router")
desk_router          = safe_import("desk", "router")
admin_router         = safe_import("admin", "router")
...
async def initialize_backend_services():
    ...
    if not RUN_INTERNAL_SCHEDULER:
        ...
        try:
            ...
            # 7. High-Frequency Live Data Polling (Continuous loop, not a cron job)
            try:
...
if arbitrage_router:     app.include_router(arbitrage_router, prefix="/api/arb", tags=["arbitrage"])
...
if scanner_router:       app.include_router(scanner_router)
if history_router:       app.include_router(history_router)
if elite_router:         app.include_router(elite_router)
if desk_router:          app.include_router(desk_router)
