...
class EdgeFeedConfig:
    ...
    min_ev_threshold: float = 0.5          # lowered from 2.0 to show more data in off-hours
    max_ev_threshold: float = 40.0         # sanity cap
    min_confidence: float = 0.51           # lowered from 0.55
    max_edge_percent: float = 30.0         # display cap
    min_line_value: float = 0.5
    max_line_value: float = 999.0
    min_odds: int = -500                   # allowed wider odds range
    max_odds: int = 500
    kelly_fraction: float = 0.25           # quarter-kelly default
    enabled_sports: List[int] = field(default_factory=lambda: [30, 31, 40, 22, 39, 41, 42, 43, 53, 54, 55])
    ...
    try:
        async with async_session_maker() as session:
            ...
            return EdgeFeedConfig(
                ...
                min_ev_threshold=float(row.get("min_ev_threshold", 0.5)),
                max_ev_threshold=float(row.get("max_ev_threshold", 40.0)),
                min_confidence=float(row.get("min_confidence", 0.51)),
                ...
                min_odds=int(row.get("min_odds", -500)),
                max_odds=int(row.get("max_odds", 500)),
                kelly_fraction=float(row.get("kelly_fraction", 0.25)),
                enabled_sports=enabled_sports if enabled_sports else [30, 31, 40, 22, 39, 41, 42, 43, 53, 54, 55],
