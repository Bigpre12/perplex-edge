...
async def populate_season_rosters():
    ...
    try:
        ...
        # Import RealSportsAPI for live data fetching
        try:
            from real_sports_api import RealSportsAPI
            real_api = RealSportsAPI()
            print("RealSportsAPI initialized for live roster fetching")
        except ImportError:
            real_api = None
            print("RealSportsAPI not found, using sample data fallbacks")

        # Check if table exists
        # ... (rest of the code)
        
        # Generate season roster data
        print("Acquiring season roster data...")
        
        final_rosters = []
        
        if real_api:
            # Attempt to fetch live rosters for major sports
            sports = ["nba", "nfl", "mlb", "nhl"]
            for sport in sports:
                print(f"Fetching {sport.upper()} rosters...")
                league_rosters = await real_api.fetch_league_rosters(sport)
                
                # Transform to DB format
                for team_name, players in league_rosters.items():
                    # Generate a pseudo-stable team_id for demo purposes
                    team_id = abs(hash(team_name)) % 1000
                    for p in players:
                        final_rosters.append({
                            'season_id': 2026,
                            'team_id': team_id,
                            'player_id': abs(hash(p.get('player_name', ''))) % 10000,
                            'jersey_number': p.get('jersey', random.randint(1, 99)),
                            'position': p.get('position', 'UTL'),
                            'is_active': p.get('is_active', True),
                            'created_at': datetime.now(timezone.utc),
                            'updated_at': datetime.now(timezone.utc)
                        })
            
            print(f"Live data acquired: {len(final_rosters)} players")

        # FALLBACK: If no live data was acquired, use original sample data
        if not final_rosters:
            print("Using original sample roster data...")
            season_rosters = [
                # NBA Season 2026
                {
                    'season_id': 2026,
