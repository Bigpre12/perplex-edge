...
function ParlayBuilderContent() {
    ...
    const fetchAvailableProps = async (sport: string) => {
        ...
        try {
            ...
            if (data.items && data.items.length > 0) {
                ...
                const fallbackRes = await fetch(API_ENDPOINTS.VALIDATION_PICKS);
    ...
    const fetchSuggestedParlays = async (sport: string) => {
        try {
            const res = await fetch(`${API_ENDPOINTS.SUGGESTED_PARLAYS}?sport_key=${sport}`);
            const data = await res.json();
    ...
    const handleExportToBook = () => {
        ...
        const summary = selectedLegs.map(l => `${l.player}: ${l.market} (${l.odds})`).join('\n');
        const text = `Lucrix Parlay Optimizer\nTotal Odds: ${totalAmericanOdds}\n\n${summary}\n\nEstimated Payout: $${estPayout.toFixed(2)}`;

        navigator.clipboard.writeText(text);
        alert("Parlay details copied to clipboard! Redirecting to sportsbook...");

        // Mock redirect to the detected best book for this specific parlay
            ...
            <aside className="hidden lg:flex flex-col w-96 shrink-0 h-fit sticky top-24">
                <div className="glass-panel rounded-2xl flex flex-col overflow-hidden border-primary/20 shadow-[0_0_50px_rgba(13,242,51,0.05)]">
                    ...
                    <div className="p-5 bg-surface border-t border-slate-800 space-y-5">
                        ...
                        <button
                            onClick={handleExportToBook}
                            className="flex-1 bg-emerald-primary text-background-dark font-black py-4 rounded-xl text-sm shadow-[0_0_30px_rgba(13,242,51,0.2)] hover:bg-emerald-primary/90 transition-all flex items-center justify-center gap-2 group"
                        >
                            EXPORT TO BOOK <ChevronRight size={18} className="group-hover:translate-x-1 transition-transform" />
                        </button>
                        <button
                            onClick={() => {
                                const summary = selectedLegs.map(l => `${l.player}: ${l.market} (${l.odds})`).join('\n');
                            ...
                            className="p-4 bg-slate-800 text-slate-400 rounded-xl hover:text-white transition-colors"
                        >
                            <Copy size={20} />
                        </button>
                    </div>
                ...
                {/* AI Parlay Advisor */}
                <div className="glass-panel p-6 rounded-2xl border-primary/10 bg-gradient-to-br from-primary/5 to-transparent mt-6">
                    <div className="flex items-center gap-3 mb-4">
                        <div className="p-2 bg-primary/20 rounded-lg text-primary">
                            <Brain size={20} />
                        </div>
                        <h3 className="text-sm font-black text-white uppercase tracking-tight">Parlay AI Advisor</h3>
                    </div>
                    <div className="space-y-3">
                        {selectedLegs.length === 0 ? (
                            <p className="text-xs text-slate-500 italic">Add legs to receive AI-driven correlation insights and leg suggestions.</p>
                        ) : (
                            <>
                                <div className="p-3 rounded-lg bg-background-dark/50 border border-primary/20 text-xs text-primary leading-relaxed">
                                    <span className="font-bold">Correlation detected:</span> High synergy between your selected {activeSport.includes('basket') ? 'NBA' : 'NHL'} props. Adding a {activeSport.includes('basket') ? 'Team Points' : 'Game Total'} under leg would maximize EV.
                                </div>
                                <div className="flex items-center justify-between p-3 rounded-lg bg-white/[0.03] border border-white/[0.05]">
                                    <div className="flex items-center gap-2">
                                        <Star size={14} className="text-amber-400" />
                                        <span className="text-[10px] font-bold text-white uppercase">Boost Suggested Leg</span>
                                    </div>
                                    <button
                                        onClick={() => addToParlay(availableProps[0])}
